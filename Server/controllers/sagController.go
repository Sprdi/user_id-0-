...
func SagIndex(c *gin.Context) {
    ...
    log.Printf("Data SAG: %+v", sags)

    // Ambil user_id dari setiap sag
    var userIDs []uint
    for _, sag := range sags {
        userIDs = append(userIDs, sag.UserID)
    }
...
func SagUserIDs(c *gin.Context) {
	var sags []models.Sag
	initializers.DB.Select("user_id").Find(&sags)
