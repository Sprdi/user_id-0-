...
export function getUserIds(callback) {
  const token = localStorage.getItem('token'); // Ambil token dari localStorage
  ...
  console.log("Token sent:", token);
  return axios
    .get(`${API_URL}/user-ids`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    })
    .then((response) => {
      callback(response.data.user_ids);
    })
    .catch((error) => {
      throw new Error(`Gagal mengambil data user IDs. Alasan: ${error.message}`);
    });
}